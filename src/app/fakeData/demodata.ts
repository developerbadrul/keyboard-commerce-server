
export const demoProducts = [
    {
        image: "https://example.com/image1.jpg",
        title: "Mechanical Gaming Keyboard",
        brand: "KeyMaster",
        availableQuantity: 50,
        price: 99.99,
        rating: 4.5,
        description: "A high-performance mechanical keyboard with RGB lighting and programmable keys."
    },
    {
        image: "https://example.com/image2.jpg",
        title: "Wireless Ergonomic Keyboard",
        brand: "ComfortType",
        availableQuantity: 30,
        price: 79.99,
        rating: 4.0,
        description: "An ergonomic keyboard designed for comfort with wireless connectivity and long battery life."
    },
    {
        image: "https://example.com/image3.jpg",
        title: "Compact Mechanical Keyboard",
        brand: "TechGear",
        availableQuantity: 20,
        price: 59.99,
        rating: 4.2,
        description: "A compact mechanical keyboard with a sleek design, perfect for small desks and gaming setups."
    },
    {
        image: "https://example.com/image4.jpg",
        title: "RGB Backlit Keyboard",
        brand: "LightWave",
        availableQuantity: 15,
        price: 49.99,
        rating: 3.8,
        description: "A stylish RGB backlit keyboard with customizable lighting effects and quiet keys."
    },
    {
        image: "https://example.com/image5.jpg",
        title: "Professional Typing Keyboard",
        brand: "TypePro",
        availableQuantity: 40,
        price: 69.99,
        rating: 4.7,
        description: "A professional-grade typing keyboard with tactile feedback and a durable build."
    }
];
